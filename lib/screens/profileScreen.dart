...
class _ProfileScreenState extends BaseState {
      ...
      child: sc(Scaffold(
        ...
        // )
                  ...
                  CachedNetworkImage(
                   ...
                   imageUrl: '${global.user.photoPath}',
                   // imageUrl: 'http://api.agri-health.org/public/photos/scaled_image_picker196561381_1682208968.jpg',
                    imageBuilder: (context, imageProvider) => Container(
                      height: MediaQuery.of(context).size.height * 0.17,
                      width: MediaQuery.of(context).size.height * 0.17,
                      alignment: Alignment.center,
  ...
  _updateUserProfileImage(File? _tImage) async {
    try {
      bool isConnected = await br!.checkConnectivity();
      if (isConnected) {
        await apiHelper?.updateProfileImage(user_image:_tImage ).then((result) {
          if (result != null) {
      ...
      print("Exception - profileScreen.dart - updateUserProfileImage():" + e.toString());
    }
  //  savedPath = await getPath();
   // setState(() { });
  }
